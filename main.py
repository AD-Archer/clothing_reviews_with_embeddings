...
def plot_tsne(tsne_results):
    ...
    for i, point in enumerate(tsne_results):
        ...
        # plt.text(point[0], point[1], str(i), fontsize=8, verticalalignment='center') # Optional: remove or keep if IDs are not too cluttered
    plt.title("t-SNE Visualization of Review Embeddings")
    plt.xlabel("t-SNE feature 1")
    plt.ylabel("t-SNE feature 2")
    plt.savefig("tsne_visualization.png") # Save the plot
    print("t-SNE plot saved to tsne_visualization.png")
    # plt.show() # Remove or comment out plt.show() for non-interactive environments
